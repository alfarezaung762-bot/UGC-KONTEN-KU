...
export function EditKombinasiModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-2xs animate-fadeIn"
      onClick={(e) => {
      ...
      <div className="w-full max-w-lg rounded-lg border border-slate-200 bg-white shadow-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 bg-slate-50">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-md bg-blue-50 border border-blue-200 text-blue-600">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          ...
          <div className="rounded-md border border-slate-200 bg-slate-50 p-3.5">
            <div className="flex items-center justify-between text-slate-700 font-bold mb-2">
              ...
              <span className="flex items-center gap-1 font-mono text-[11px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded">
            ...
            <div className="flex flex-wrap gap-1.5">
              {kombinasi.gerakanList?.map((item, idx) => {
                ...
                return (
                  <span
                    key={item.id || idx}
                    className="inline-flex items-center gap-1 rounded bg-white border border-slate-200 px-2 py-1 text-[11px] text-slate-800 font-mono shadow-2xs"
                  >
                    <span className="text-slate-400 font-bold">#{idx + 1}</span>
                    {g && (
                      <span className="px-1 py-0.2 rounded text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200">
                        {g.kode}
                      </span>
                    )}
                    <span className="text-slate-800 truncate max-w-[120px] font-semibold">
                      {g?.nama || "Gerakan"}
                    </span>
          ...
          <div>
            <label className="text-slate-700 font-semibold block mb-1">
              Nama Preset Kombinasi <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
              required
              ...
              className="w-full rounded-md border border-slate-300 bg-white px-3.5 py-2 text-slate-900 text-xs focus:border-blue-500 focus:ring-2 focus:ring-blue-100 focus:outline-none transition-colors"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-md border border-slate-300 bg-white p-3 text-slate-900 text-xs focus:border-blue-500 focus:ring-2 focus:ring-blue-100 focus:outline-none transition-colors leading-relaxed"
            />
            <p className="mt-1 text-[10px] text-slate-500">
          ...
          <div className="pt-3 border-t border-slate-200 flex items-center justify-end gap-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-1.5 rounded-md bg-blue-600 px-4 py-2 text-xs font-bold text-white hover:bg-blue-700 active:scale-95 disabled:opacity-50 transition-all shadow-xs"
            >
