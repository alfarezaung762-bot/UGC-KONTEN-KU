...
export function GerakanSidebar({
  ...
  return (
    <div className="flex h-full flex-col border-r border-slate-200 bg-white">
      ...
      <div className="border-b border-slate-200 p-4 space-y-3 bg-slate-50/50">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-blue-600" />
            <h2 className="text-sm font-bold text-slate-900">Bank Gerakan</h2>
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-md bg-blue-50 px-2.5 py-1.5 text-xs font-semibold text-blue-700 border border-blue-200 hover:bg-blue-600 hover:text-white transition-all shadow-2xs"
        ...
        {/* Search Bar (Bootstrap form-control style) */}
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-md border border-slate-300 bg-white py-1.5 pl-8 pr-7 text-xs text-slate-900 placeholder-slate-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery("")}
              className="absolute right-2.5 top-1.5 text-sm text-slate-400 hover:text-slate-600"
            >
              ×
            </button>
          )}
        ...
        <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-4 gap-1">
            <button
              ...
              className={`rounded px-1.5 py-1 text-[10px] font-semibold transition-all text-center ${
                comboFilter === "ALL"
            ...
            <button
              ...
              className={`rounded px-1.5 py-1 text-[10px] font-semibold transition-all text-center flex items-center justify-center gap-1 ${
                comboFilter === "USED"
            ...
            >
              ...
              <span>Kombinasi ({comboStats.usedCount})</span>
            </button>
          ...
          <div className="flex items-center gap-1">
            {["ALL", "UNIVERSAL", "KONDISIONAL"].map((t) => (
              <button
                ...
                className={`rounded px-1.5 py-0.5 text-[10px] uppercase font-semibold transition-all ${
                  filterType === t
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2.5 bg-slate-50/30">
        {filteredGerakan.length === 0 ? (
          <div className="rounded-lg border border-dashed border-slate-300 p-6 text-center bg-white">
            <p className="text-xs text-slate-500">Tidak ada gerakan yang cocok dengan filter saat ini.</p>
            <button
              onClick={() => {
                setSearchQuery("");
          ...
          filteredGerakan.map((g) => {
            ...
            return (
              <div
                key={g.id}
                className={`group relative rounded-lg border bg-white p-3 transition-all hover:border-blue-400 hover:shadow-xs ${
                  hasCombinations ? "border-amber-200" : "border-slate-200"
                } ${
                  isJustAdded ? "ring-2 ring-emerald-500 bg-emerald-50/40" : ""
                }`}
              >
                {/* Card Header */}
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex items-center gap-1.5 flex-wrap">
                      {combos.length > 0 ? (
                        <span
                          className="inline-flex items-center gap-1 rounded bg-amber-50 border border-amber-200 px-1.5 py-0.5 text-[10px] font-semibold text-amber-800"
                        ...
                        <span className="inline-flex items-center gap-1 rounded bg-slate-100 border border-slate-200 px-1.5 py-0.5 text-[10px] font-medium text-slate-600">
                      ...
                      {timelineCount > 0 && (
                        <span className="inline-flex items-center gap-1 rounded bg-blue-50 border border-blue-200 px-1.5 py-0.5 text-[10px] font-bold text-blue-700">
                  ...
                  {/* Add to timeline Button (Bootstrap btn-primary style) */}
                  <button
                    onClick={() => handleAdd(g)}
                    className={`flex h-7 items-center gap-1 rounded-md px-2.5 text-xs font-semibold transition-all shrink-0 ${
                      isJustAdded
                        ? "bg-emerald-600 text-white"
                        : "bg-blue-600 text-white hover:bg-blue-700 active:scale-95 shadow-2xs"
                    }`}
                ...
                <div className="mt-2.5 flex items-center justify-between pt-1.5 border-t border-slate-100 text-[10px]">
                  <span className="text-slate-500 flex items-center gap-1 truncate max-w-[190px]">
                    ...
                    {combos.length > 0 ? (
                      ...
                      <span className="text-amber-800 font-medium truncate">{g.dipakaiDi}</span>
                    ) : (
                  ...
                  <button
                    ...
                    className="flex items-center gap-0.5 text-blue-600 hover:text-blue-800 font-medium transition-colors shrink-0 ml-1"
                  >
                ...
                {/* Expanded Details */}
                {isExpanded && (
                  <div className="mt-2 space-y-2 rounded-md bg-slate-50 p-2.5 text-xs border border-slate-200 text-slate-700 animate-fadeIn">
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-slate-800 text-[11px] mt-0.5 leading-relaxed">
                        {g.deskripsi}
                      </p>
                    </div>

                    {g.kondisi && (
                      <div className="rounded bg-amber-50 p-1.5 border border-amber-200 text-amber-900 text-[11px] font-medium">
                        ⚠️ Syarat: {g.kondisi}
                      </div>
                    )}
                    ...
                    <div className="pt-2 border-t border-slate-200">
                      {combos.length > 0 ? (
                        <div className="rounded-md bg-amber-50/70 border border-amber-200 p-2.5 space-y-2">
                          ...
                          <div className="space-y-1.5">
                            {combos.map((c) => {
                              ...
                              return (
                                <div
                                  key={c.id}
                                  className="flex items-center justify-between rounded bg-white px-2.5 py-1.5 text-xs text-slate-800 border border-slate-200 shadow-2xs"
                                >
                                  <div className="flex items-center gap-1.5 truncate max-w-[160px]">
                                    ...
                                    <span className="font-semibold truncate text-[11px]" title={c.nama}>
                                      {c.nama}
                                    </span>
                                  </div>
                                  ...
                                  <div className="flex items-center gap-1.5 shrink-0">
                                    ...
                                    {onLoadKombinasi && (
                                      <button
                                        ...
                                        className="rounded bg-blue-600 px-2 py-0.5 text-[10px] font-bold text-white hover:bg-blue-700 transition-colors shadow-2xs"
                        ...
                        <div className="rounded bg-amber-50 border border-amber-200 p-2 text-[11px] text-amber-900">
                          📌 Tercatat pada aturan default: <strong>{g.dipakaiDi}</strong>
                        </div>
                      ) : (
                        <div className="rounded bg-slate-100 border border-slate-200 p-2 text-[11px] text-slate-600 flex items-center gap-1.5">
